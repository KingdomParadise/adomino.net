<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <meta name="csrf-token" content="{{ csrf_token() }}">
    <meta name="base-url" content="{{ url('/') }}">
    <meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
    <title>Adomino Backend</title>
    <link rel="shortcut icon" type="image/x-icon" href="{{ asset('img/favicon.ico') }}">
    <link rel="stylesheet"
          href="https://fonts.googleapis.com/css?family=Source+Sans+Pro:300,400,400i,700&display=fallback">
    <link rel="stylesheet" href="{{url('themes/fontawesome-free/css/all.min.css')}}">
    <link rel="stylesheet" href="{{url('themes/data-table/css/data-table-bootstrap.css')}}">
    <link rel="stylesheet" href="{{url('themes/select2/css/select2.min.css')}}">
    <link rel="stylesheet" href="{{url('themes/select2-bootstrap4-theme/select2-bootstrap4.min.css')}}">
    <link rel="stylesheet" href="{{url('themes/daterangepicker/daterangepicker.css')}}">
    <link rel="stylesheet" href="{{url('themes/tempusdominus-bootstrap-4/css/tempusdominus-bootstrap-4.min.css')}}">
    <link rel="stylesheet" href="{{url('css/adomino-theme.min.css')}}">
    <link rel="stylesheet" href="{{url('css/adomino.css')}}">
</head>
<body class="hold-transition sidebar-mini">
<div class="wrapper">
    {{--style="width: 100%"--}}
    <nav class="main-header navbar navbar-expand navbar-white navbar-light">
        <ul class="navbar-nav ml-auto">
            {{--<li class="nav-item">--}}
            {{--<a class="nav-link" data-widget="control-sidebar" data-slide="true" href="{{ route('logout') }}"--}}
            {{--onclick="event.preventDefault();--}}
            {{--document.getElementById('logout-form').submit();" role="button">--}}
            {{--<i class="fas fa-sign-out-alt"></i>&nbsp;&nbsp;Abmelden--}}
            {{--</a>--}}
            {{--</li>--}}
            <li class="nav-item dropdown">
                <a class="nav-link" data-toggle="dropdown" href="#">
                    {{ Auth::user()->name }} &nbsp;<i class="fas fa-angle-down"></i>
                </a>
                <div class="dropdown-menu dropdown-menu-right">
                    <a class="nav-link" data-widget="control-sidebar" data-slide="true" href="{{ route('logout') }}"
                       onclick="event.preventDefault();
            document.getElementById('logout-form').submit();" role="button">
                        <i class="fas fa-sign-out-alt"></i>&nbsp;&nbsp;Abmelden
                    </a>
                </div>
            </li>
            <form id="logout-form" action="{{ route('logout') }}" method="POST" class="d-none">
                @csrf
            </form>
        </ul>
    </nav>
    <aside class="main-sidebar sidebar-dark-primary elevation-4">
        <a href="{{url('/')}}" class="brand-link">
            {{--<span class="brand-text h3"><b>Adomino.</b>net</span>--}}
            <img src="{{url('img/white_logo.png')}}" style="width: 60%; margin-left: 45px; margin-right: 15px;"/>
        </a>
        <div class="sidebar">
            <nav class="mt-2">
                <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu"
                    data-accordion="false">
                    <li class="nav-item">
                        <a href="{{route('dashboard')}}" class="nav-link @if($sidebar == 'Dashboard') active @endif">
                            <i class="nav-icon fas fa-tachometer-alt"></i>
                            <p>
                                Dashboard
                            </p>
                        </a>
                    </li>
                    <li class="nav-item @if($sidebar == 'Anfragen') menu-open @endif">
                        <a href="#" class="nav-link">
                            <i class="nav-icon fas fa-balance-scale"></i>
                            <p>
                                Verkauf
                                <i class="right fas fa-angle-left"></i>
                            </p>
                        </a>
                        <ul class="nav nav-treeview">
                            <li class="nav-item">
                                <a href="{{route('inquiry')}}"
                                   class="nav-link @if($sidebar == 'Anfragen') active @endif">
                                    <i class="nav-icon fas"></i>
                                    <p>Anfragen</p>
                                </a>
                            </li>
                        </ul>
                    </li>
                    <li class="nav-item @if($sidebar == 'Domains' || $sidebar == 'Fehler-Domains') menu-open @endif">
                        <a href="#" class="nav-link">
                            <i class="nav-icon fas fa-funnel-dollar"></i>
                            <p>
                                Domains
                                <i class="right fas fa-angle-left"></i>
                            </p>
                        </a>
                        <ul class="nav nav-treeview">
                            <li class="nav-item">
                                <a href="{{route('domain')}}" class="nav-link @if($sidebar == 'Domains') active @endif">
                                    <i class="nav-icon fas"></i>
                                    <p>Domainliste</p>
                                </a>
                            </li>
                            <li class="nav-item">
                                <a href="{{route('not-found-domains')}}"
                                   class="nav-link @if($sidebar == 'Fehler-Domains') active @endif">
                                    <i class="nav-icon fas"></i>
                                    <p>Fehler-Domains</p>
                                </a>
                            </li>
                        </ul>
                    </li>
                    <li class="nav-item @if($sidebar == 'Aufrufe IP' || $sidebar == 'Aufrufe Tag' || $sidebar == 'STATISTIK') menu-open @endif">
                        <a href="#" class="nav-link">
                            <i class="nav-icon fas fa-chart-bar"></i>
                            <p>
                                Statistiken
                                <i class="right fas fa-angle-left"></i>
                            </p>
                        </a>
                        <ul class="nav nav-treeview">
                            <li class="nav-item">
                                <a href="{{route('visits')}}"
                                   class="nav-link @if($sidebar == 'Aufrufe IP') active @endif">
                                    <i class="nav-icon fas"></i>
                                    <p>Aufrufe IP</p>
                                </a>
                            </li>
                            <li class="nav-item">
                                <a href="{{route('daily-visit')}}"
                                   class="nav-link @if($sidebar == 'Aufrufe Tag') active @endif">
                                    <i class="nav-icon fas"></i>
                                    <p>Aufrufe Tag</p>
                                </a>
                            </li>
                            <li class="nav-item">
                                <a href="{{route('statistics')}}"
                                   class="nav-link @if($sidebar == 'STATISTIK') active @endif">
                                    <i class="nav-icon fas"></i>
                                    <p>Aufrufe Detail</p>
                                </a>
                            </li>
                        </ul>
                    </li>
                    <li class="nav-item @if($sidebar == 'Logos') menu-open @endif">
                        <a href="#" class="nav-link">
                            <i class="nav-icon fas fa-archway"></i>
                            <p>
                                Webseite
                                <i class="right fas fa-angle-left"></i>
                            </p>
                        </a>
                        <ul class="nav nav-treeview">
                            <li class="nav-item">
                                <a href="{{route('logo')}}" class="nav-link @if($sidebar == 'Logos') active @endif">
                                    <i class="nav-icon fas"></i>
                                    <p>Kundenlogos</p>
                                </a>
                            </li>
                        </ul>
                    </li>
                    <li class="nav-item @if($sidebar == 'Benutzer') menu-open @endif">
                        <a href="#" class="nav-link">
                            <i class="nav-icon fas fa-cogs"></i>
                            <p>
                                Einstellungen
                                <i class="right fas fa-angle-left"></i>
                            </p>
                        </a>
                        <ul class="nav nav-treeview">
                            <li class="nav-item">
                                <a href="{{route('home')}}" class="nav-link @if($sidebar == 'Benutzer') active @endif">
                                    <i class="nav-icon fas"></i>
                                    <p>Benutzer</p>
                                </a>
                            </li>
                        </ul>
                    </li>
                    {{--menu-open--}}
                    {{--<li class="nav-item">--}}
                    {{--<a href="#" class="nav-link">--}}
                    {{--<i class="nav-icon fas fa-tachometer-alt"></i>--}}
                    {{--<p>--}}
                    {{--Ressourcen--}}
                    {{--<i class="right fas fa-angle-left"></i>--}}
                    {{--</p>--}}
                    {{--</a>--}}
                    {{--<ul class="nav nav-treeview">--}}
                    {{--<li class="nav-item">--}}
                    {{--<a href="{{route('inquiry')}}"--}}
                    {{--class="nav-link @if($sidebar == 'Anfragen') active @endif">--}}
                    {{--<i class="far fa-circle nav-icon text-warning"></i>--}}
                    {{--<p>Anfragen</p>--}}
                    {{--</a>--}}
                    {{--</li>--}}
                    {{--<li class="nav-item">--}}
                    {{--<a href="{{route('home')}}" class="nav-link @if($sidebar == 'Benutzer') active @endif">--}}
                    {{--<i class="far fa-circle nav-icon text-warning"></i>--}}
                    {{--<p>Benutzer</p>--}}
                    {{--</a>--}}
                    {{--</li>--}}
                    {{--<li class="nav-item">--}}
                    {{--<a href="{{route('domain')}}" class="nav-link @if($sidebar == 'Domains') active @endif">--}}
                    {{--<i class="far fa-circle nav-icon text-warning"></i>--}}
                    {{--<p>Domains</p>--}}
                    {{--</a>--}}
                    {{--</li>--}}
                    {{--<li class="nav-item">--}}
                    {{--<a href="{{route('not-found-domains')}}"--}}
                    {{--class="nav-link @if($sidebar == 'Fehler-Domains') active @endif">--}}
                    {{--<i class="far fa-circle nav-icon text-warning"></i>--}}
                    {{--<p>Fehler-Domains</p>--}}
                    {{--</a>--}}
                    {{--</li>--}}
                    {{--<li class="nav-item">--}}
                    {{--<a href="{{route('logo')}}" class="nav-link @if($sidebar == 'Logos') active @endif">--}}
                    {{--<i class="far fa-circle nav-icon text-warning"></i>--}}
                    {{--<p>Logos</p>--}}
                    {{--</a>--}}
                    {{--</li>--}}
                    {{--</ul>--}}
                    {{--</li>--}}
                    {{--<li class="nav-item">--}}
                    {{--<a href="#" class="nav-link">--}}
                    {{--<i class="nav-icon fas fa-chart-pie"></i>--}}
                    {{--<p>--}}
                    {{--STATISTIK--}}
                    {{--<i class="right fas fa-angle-left"></i>--}}
                    {{--</p>--}}
                    {{--</a>--}}
                    {{--<ul class="nav nav-treeview">--}}
                    {{--<li class="nav-item">--}}
                    {{--<a href="{{route('visits')}}"--}}
                    {{--class="nav-link @if($sidebar == 'Aufrufe IP') active @endif">--}}
                    {{--<i class="far fa-circle nav-icon text-info"></i>--}}
                    {{--<p>Aufrufe IP</p>--}}
                    {{--</a>--}}
                    {{--</li>--}}
                    {{--<li class="nav-item">--}}
                    {{--<a href="{{route('daily-visit')}}"--}}
                    {{--class="nav-link @if($sidebar == 'Aufrufe Tag') active @endif">--}}
                    {{--<i class="far fa-circle nav-icon text-info"></i>--}}
                    {{--<p>Aufrufe Tag</p>--}}
                    {{--</a>--}}
                    {{--</li>--}}
                    {{--<li class="nav-item">--}}
                    {{--<a href="{{route('statistics')}}"--}}
                    {{--class="nav-link @if($sidebar == 'STATISTIK') active @endif">--}}
                    {{--<i class="far fa-circle nav-icon text-info"></i>--}}
                    {{--<p>STATISTIK</p>--}}
                    {{--</a>--}}
                    {{--</li>--}}
                    {{--</ul>--}}
                    {{--</li>--}}
                </ul>
            </nav>
        </div>
    </aside>

    @yield('content')
</div>
<div id="adominoModalContent">

</div>
<script src="{{url('themes/jquery/jquery.min.js')}}"></script>
<script src="{{url('themes/bootstrap/js/bootstrap.bundle.min.js')}}"></script>
<script src="{{url('themes/data-table/js/jquery.dataTables.min.js')}}"></script>
<script src="{{url('themes/data-table/js/dataTables.bootstrap4.min.js')}}"></script>
<script src="{{url('themes/select2/js/select2.full.min.js')}}"></script>
<script src="{{url('themes/moment/moment.min.js')}}"></script>
<script src="{{url('themes/daterangepicker/daterangepicker.js')}}"></script>
<script src="{{url('themes/tempusdominus-bootstrap-4/js/tempusdominus-bootstrap-4.min.js')}}"></script>
<script src="{{url('themes/inputmask/jquery.inputmask.min.js')}}"></script>
<script src="{{url('js/adomino-theme.min.js')}}"></script>
<script src="{{url('js/adomino-common.js')}}"></script>
<script src="{{url('js/adomino.js')}}"></script>
</body>
</html>
